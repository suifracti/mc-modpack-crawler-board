...
- **审计功能总项数**：`73` 项 (覆盖 16 个业务领域)
- **状态分布汇总 (Phase 3G-F 审计后)**：
  - **VERIFIED**：`47` 项 (64.4%) — 确证真实、具备完备契约的可靠功能（包含 TIME-CURSEFORGE-02、MODREL-MCMOD-01、DIDX-MCMOD-01、SEARCH-MCMOD-DESC-01、SEARCH-MCMOD-MULTIWORD-01、SEARCH-MCMOD-REASON-01、SEARCH-MCMOD-REASON-SOURCE-01、回滚与发布完整性契约 REL-ROLLBACK-*，以及 BILI-GRP-01 / **BILI-GRP-MERGE-BENCH-01** / **BILI-GRP-PRECISION-BENCH-01**）
  - **SUSPECT**：`19` 项 (26.0%) — 保留审慎标记（BBSMC 时序、Modrinth 聚合时间与版本标识、**BILI-GRP-02 宽口径误合并安全性**、**BILI-GRP-03 剩余 6 例 false split**）
  - **WRONG**：`0` 项 (0.0%) — Phase 3G-E 确认的 `BILI-GRP-03` 系统性误拆分已在 Phase 3G-F 大幅修复（Recall $0.1250 \rightarrow 0.7500$，False Split $21 \rightarrow 6$），降级为 SUSPECT；剩余 6 例根因明确且无法在不牺牲精度的前提下修复
  - **UNKNOWN**：`7` 项 (9.6%) — 保持显式未确定（含 RELDATE-CURSEFORGE-01 及 MODSEM-MCMOD-01）
...
| Feature ID | 平台 | 用户可见功能 / 结论 | 原始平台来源 | 推导算法与逻辑 | 缺失值处理 | Golden Samples | 最终状态 | 备注 / 风险 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `BILI-GRP-01` | Bilibili | 同 UP 主多期视频自动合为一个卡片 (Multi-video Collapsing) | 936 条 Bilibili 视频标题与 UP 主 ID | 同作者 + 共享 identity run 合并为同一卡片（Phase 3G-F 起由 `src/domain/bilibiliGrouping.ts` 单一来源决策） | 泛名/短 key 退化为单视频卡片 | UP `懂嗎懂嗎`: 8 视频合为 1 组; UP `AC6_` 云游四海: 9 视频合为 1 组; UP `ConfectionaryQwQ` Horizon: 6 视频合为 1 组 | **`VERIFIED`** | 核心机制正确。**注意**：`53 raw` 是 Flat Mode 不变量；`47 grouped` 只是修复前的一次快照，**不是正确性 Golden**，Phase 3G-F 修复 false split 后已变化（53 raw → 36 组）。 |
| `BILI-GRP-02` | Bilibili | 标题去重关键词引发的错误合并 (False Merge) | 视频标题 | **两级 identity/episode 分组**（`apps/web/src/domain/bilibiliGrouping.ts`）：作者作用域前缀 `authorKey::`；泛名/短 key（$\le$ 3 字或属泛名白名单）守卫强制 `__raw_<bvid>`；identity run 必须由**非噪声 token** 构成（题材词、渠道术语、更新词、模组名、源游戏名均不可锚定） | 退化为单视频卡片 | **Phase 3G-F 基准**：22 个 hard negative 在**孤立与全量 936 两种上下文**下均正确分开（0 false merge）；含 phase 指定的 `黑金` 重建对照，清洗后均为 `生存` → 各自 `__raw_` | **`SUSPECT`** | **审慎维持 SUSPECT（宽口径误合并安全性尚未穷尽）**：本轮冻结 benchmark 内 False Merge = 0.0000，且**全量 population 上下文同样 0 违规**；但审计过程中确实发现并修复了两类 corpus 未覆盖的真实误合并（`机械动力` 模组名锚定 → 命运齿轮/月亮工厂；`地下城` 源游戏名锚定 → 幻想的地下城/史诗的地下城）。这证明「0 false merge」只在**已枚举的 negative 形态**下成立，泛化安全性仍需更多形态覆盖，故不升为 VERIFIED。窄口径结论见 `BILI-GRP-PRECISION-BENCH-01`。 |
| `BILI-GRP-03` | Bilibili | 包含版本特性描述导致的拆分 (False Split) | 视频标题 | 两级模型：`identityKey`（跨期稳定、可判别的 token run）+ `episodeResidue`（版本/更新日志残渣）。同作者两期标题若共享一条 identity run 即判为同一系列，**不再要求 key 完全相同或互为子串** | 无法归纳到同一 identity 时退化为独立卡片 | **Phase 3G-F 修复后**：`Horizon 地平线` v1.2.0→v2.1.0 由 6 张卡片合为 **1 组**；21 个旧 false split 中 **15 个已修复**；`群峦野望 1.1/1.2/1.3` 由 3 张合为 1 组 | **`SUSPECT`** | **大幅改善但未清零**：Merge Recall $0.1250 \rightarrow$ **0.7500**（dev 0.7222 / holdout 0.8333，holdout 不低于 dev，无过拟合迹象），False Split $21 \rightarrow$ **6**，Precision 保持 **1.0000**。剩余 6 例根因明确且**不可在不牺牲精度的前提下修复**：3 例包名本身 $\le$ 2 字（`化龍`/`涅槃`/`芦苇`），2 例包名完全由题材词构成（`沉浸战斗` → 清洗后 `沉浸`，被泛名守卫正确拦下），1 例包名恰为 3 字被守卫截断（`咒次元`）。**保留少量 split 优于引入误合并**，故记 SUSPECT 而非 VERIFIED。 |
...
| `BILI-GRP-PRECISION-BENCH-01` | Bilibili | 冻结 benchmark 上是否观察到误合并 | 22 个 hard negative control + 全量 936 上下文 | 逐案调用真实实现；另在**全量 936 条**上重跑同一批 negative，验证 production 真实上下文（搜索过滤后的集合）下仍不误合并 | — | `negative_regression = 22/22 separate`；population-context violations = **0**；`new_false_merges = []` | **`VERIFIED`** | 窄口径结论：**在本轮冻结的 negative 语料及其全量上下文下，未观察到任何误合并**（False Merge Rate 0.0000，Precision 1.0000）。该结论不推广为「算法整体无误合并」——见 `BILI-GRP-02` 的 SUSPECT 说明。 |
...
11. **`BILI-GRP-03`** [**Phase 3G-F 已大幅修复，降级为 SUSPECT**]: 同包多期视频被系统性拆分。Phase 3G-E 实测 Merge Recall = 0.1250 / False Split = 21；Phase 3G-F 以两级 identity/episode 模型修复后：Recall = **0.7500**（dev 0.7222 / holdout 0.8333）、False Split = **6**、Precision 保持 **1.0000**、22/22 negative 在孤立与全量两种上下文下均不误合并。剩余 6 例根因明确（包名 ≤ 2 字、包名全为题材词、包名恰 3 字被泛名守卫截断），**无法在不牺牲精度的前提下修复**。证据：`docs/audit/BILIBILI_GROUPING_AUDIT.md`、`build/audit/bilibili_grouping_remediation.json`。
...
5. **`BILI-GRP-02`**（宽口径误合并安全性）: 冻结 benchmark 内 False Merge = 0 且全量上下文 0 违规，但 Phase 3G-F 审计中确实发现并修复了两类 corpus 未覆盖的真实误合并（模组名 `机械动力` 锚定、源游戏名 `地下城` 锚定）。说明「0 false merge」目前只在**已枚举的 negative 形态**下成立，泛化安全性需要更多形态覆盖后才可升级。
6. **`BILI-GRP-03`**（剩余 false split）: 6 例未修复，根因是包名本身过短或完全由题材词构成（`化龍`/`涅槃`/`芦苇`/`沉浸战斗`/`咒次元`）。修复它们需要放宽泛名守卫或降低 identity 阈值，会直接牺牲 precision，**按「精度优先」原则主动保留**。
