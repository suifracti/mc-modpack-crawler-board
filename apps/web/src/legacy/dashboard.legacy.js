/**
 * Refactored Legacy Dashboard Module (Architecture V2 — Phase 3A).
 * Decoupled from God Object:
 * - Data loading delegated to LegacySidecarLoader & Repository
 * - Pure string/HTML utilities imported from src/utils/html
 * - Formatting utilities imported from src/utils/format
 * - Minecraft version extraction imported from src/domain/minecraft
 * - Pack name normalization imported from src/domain/packName
 * - Theme state managed by src/state/theme
 */
...
export function initLegacyDashboard() {
    ...
    /* [Migrated to src/utils/html.ts: escHtml, escAttrJs] */
function toggleMultiSelect(selector, val) {
    ...
    /* [Migrated to src/data/LegacySidecarLoader.ts & platformRegistry.ts] */
    var PLATFORMS = PLATFORM_CONFIGS;
...
function onPlatformLoaded(platId) {
    ...
    function loadPlatformScript(platId, callback) {
        LegacySidecarLoader.load(platId).then(function() {
            onPlatformLoaded(platId);
            if (callback) callback();
        }).catch(function(err) {
            console.error("加载 " + platId + " 失败:", err);
        });
    }

    function startIdlePrefetch() {
        LegacySidecarLoader.startIdlePrefetch();
    }
    ...
    /* [Migrated to src/domain/minecraft.ts: extractMcVersion as extractVersion] */
    ...
    /* [Migrated to src/state/theme.ts: Theme listener 1] */
function showToast(msg) {
    ...
    /* [Migrated to src/state/theme.ts: Theme listener 2] */
...
/* ══════════════ 1. DataTables 自定义多条件过滤器 ══════════════ */
    ...
    /* [Migrated to src/utils/format.ts: fmtBigNum] */
    ...
    /* [Migrated to src/utils/format.ts: formatVFileSize] */
