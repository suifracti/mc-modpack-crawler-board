 ...
 * -----------------------------
 * The 涅槃 case (uploader 墨言eclipse, 7 records, 5 group keys) is the canonical
 * example: two of its groups were flagged by the detector, but the members ARE
 * the same pack. The real defect is a false SPLIT. Phase 3G-F-A already learned
 * this lesson; the ledger encodes it instead of re-learning it.
 *
...
const VERDICTS = {
  // =========================================================================
  // REAL FALSE MERGES - the 7 known from Phase 3G-F-A. ALL MUST SURVIVE.
  // =========================================================================
  '一个小寂哦::星辉死神': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'high',
    note: '神器收集计划 / 无尽幸运方块大陆 / 全网最全神器 = 多个不同包；“星辉死神”是 Boss 名，出现在每个包的模组清单里。',
  },
  '一个小寂哦::四叶草': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'high',
    note: '新泰坦生物整合包 / 执行之龙生存整合包 是两个不同包；“四叶草”是物品名。',
  },
  '一个小寂哦::各大主播同款': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'high',
    note: '幸运方块大全 / 超困难神器泰坦随机合成 是两个不同包；“各大主播同款”是宣传口号。',
  },
  '墨言eclipse::颠覆性的': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'high',
    note: '摄影奇境 / 千界万锻 是两个不同包；“颠覆性的”是形容词，出现于两个包的宣传语。',
  },
  '原界环::or not': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'high',
    note: 'Minecraft or Not: Girl&Gun / Maiden or not 是两个不同包（后者明确写明“魔改自 locknar 的 Minecraft or not”）；“or not”是命名后缀。',
  },
  '叙利亚自爆民兵::voxy': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'high',
    note: '《你好，新蒸程》与《你好，新世代》是两个独立命名的包，唯一共同点是都用了 voxy 渲染模组。voxy 是通用模组名，不构成包标识。',
  },
  'tibsalta::难度驱动': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'high',
    note: '《抗争之际0.6》与《旅途痕迹0.5》是两个不同包；包名在方括号外，【难度驱动】只是系列标签，规则把方括号内的标签当成了 anchor。',
  },

  // =========================================================================
  // NEW FALSE MERGES found by the widened detector (beyond the known 7).
  // =========================================================================
  '一个小寂哦::怪物大乱斗': {
    verdict: 'REAL_FALSE_MERGE', confidence: 'medium',
    note: '“怪物大乱斗 手机版”与“怪物大乱斗：重生”是两次独立发布，后者是明确的高版本重做（还原1.7怪物大乱斗）。同 uploader 但版本代际不同，判为不同包。注意与 LEGITIMATE 的 “怪物大乱斗重生” 组区分。',
  },
  '一个小寂哦::各大主播同款__DUP': {
    verdict: null, confidence: null, note: 'placeholder-never-used',
  },
  '落烟雨辰呀::诡厄 使徒': {
    verdict: 'AMBIGUOUS', confidence: 'low',
    note: '“诡厄：使徒”1.0.6/1.0.7 与另一组“诡厄使徒”1.1.0：两组几乎肯定是同一个包，但被拆成两组。本组内部两条是同一包 → 不是误合并。真正的缺陷是 SPLIT（见 false_split_indicators）。',
  },
  '落烟雨辰呀::诡厄使徒': {
    verdict: 'FALSE_SPLIT_INDICATOR', confidence: 'medium',
    note: '本组两条为同一包（1.1.0 发布 + 诸神黄昏宣传）。与“诡厄 使徒”(1.0.6/1.0.7) 应属一个逻辑包却被拆成两组 → 这是 under-merge 症状，不是误合并。',
  },
  '-阳春面面-::青春复兴': {
    verdict: 'AMBIGUOUS', confidence: 'low',
    note: '“格雷青春版?青春复兴 0.7.1”与“蔚蓝档案超大型科技冒险整合包：青春复兴 早期版本”是否同一包的两次改版，标题无法判定。不计数。',
  },

  // =========================================================================
  // FALSE SPLIT / UNDER-MERGE SYMPTOMS
  // Members ARE the same pack; the defect is that the pack got split. These
  // MUST NOT be counted as false merges (see the 涅槃 note in 3G-F-A).
  // =========================================================================
  '墨言eclipse::沉浸 深度 a 咒镰双生': {
    verdict: 'FALSE_SPLIT_INDICATOR', confidence: 'high',
    note: '两条均为 涅槃 v0.1.6 / v0.1.5（同一包）。anchor 落在 tag [沉浸战斗/深度魔改/ARPG/咒镰双生] 上是“坏 anchor”，但成员确定同包 → 不是误合并。真正问题是涅槃被拆成 5 组。',
  },
  '墨言eclipse::未尽之路涅槃': {
    verdict: 'FALSE_SPLIT_INDICATOR', confidence: 'high',
    note: '两条均为 涅槃（预发布 / 发布前预热），同一包。“未尽之路涅槃”是阶段性标题词被当作 anchor → 属于 false split。',
  },
  '墨言eclipse::拒绝同质': {
    verdict: 'LEGITIMATE_SAME_PACK', confidence: 'medium',
    note: '三条均为『未尽之路 / Unfinished Path』同一包（0.13 / 主线完成 / 1.0 正式发布）。“拒绝同质！抵制水槽！”是该包的固定宣传口号 → anchor 是 slogan，但成员同包。与“未尽之路涅槃”组的关联属于跨包（涅槃 ≠ 未尽之路），不应合并。',
  },

  // =========================================================================
  // LEGITIMATE_SAME_PACK
  // =========================================================================

  // ---- same pack, multiple versions ----
  ...
  'arr-c6h6::旧世界': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为「旧世界」v0.3/v0.4Beta 同一包；anchor 是包名。' },
  ...
  '墨竹ギ::史诗的地下城 dungeons of fantasy': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为 Dungeons Of Fantasy 同一包 7.0/7.2；anchor 是包名。' },
  '墨竹ギ::深渊之诗': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '3 条均为《深渊之诗 / Poetry of the Abyss》同一包 2.0/2.2；anchor 是包名。' },
  ...
  '流霜雾影::愚者版本大': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为「愚者」同一包 v0.1/v0.2；anchor 落在“愚者版本大更新”上，但成员同包。' },
  ...
  '不知名的莫理沙::勇者之章': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '4 条均为《勇者之章Ⅲ》同一包（异界法师/星月枪姬是其子版本名）；anchor 是系列名。' },
  ...
  '一个小寂哦::怪物大乱斗重生': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'medium', note: '3 条均为“怪物大乱斗重生 / 怪物大乱斗：重生”同一包（重制版的多次更新）。与 REAL_FALSE_MERGE 的“怪物大乱斗”组区分：那一组把手机版和重生版错并。' },
  ...
  '在职玩家jostar::去吧 方可梦大师': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '3 条均为《去吧，方可梦大师》同一包 1.7/3.0/4.0；anchor 是包名。' },
  ...
  'plaudite_::scarlet': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为 Scarlet Adventure 绯红冒险同一包（v2.0 / 介绍）；anchor 是包名。' },
  'pork猪排::化龍 金鳞岂是池中物 一遇风云便化龙': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为《化龍》同一包（1.0 正式版 / 发布）；后半是固定诗句宣传语。' },
  ...
  '一个小寂哦::重回 之巅': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为该 uploader 的“最全拔刀剑”系列（3.0 / 重逢 2.0），“重回1.7拔刀剑之巅”是两次共用的口号 → 同一系列包。' },
  '共轭o::武道宗师': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为《武道宗师》同一包（1.0 完整版 / 发布）；anchor 是包名。' },
  '啊liu22::简单群峦': { verdict: 'LEGITIMATE_SAME_PACK', confidence: 'high', note: '2 条均为《简单群峦》同一包 v3/2.0；sharedURL 1 + sharedQQ 672326817 佐证。' },
  ...
  // ---- LEGITIMATE but specifically worth flagging as "thin anchor risk" ----
  '一个小寂哦::怪物大乱斗__NEVER': { verdict: null, confidence: null, note: 'placeholder-never-used' },
};
...
const UNDERMERGE_VERDICTS = {
  ...
  '墨言eclipse||墨言eclipse::未尽之路涅槃|墨言eclipse::拒绝同质': {
    verdict: 'NOT_UNDER_MERGE', confidence: 'high',
    note: '否决：此簇由“未尽之路”桥接，但「未尽之路涅槃」= 涅槃包（预发布/预热），「拒绝同质」= 未尽之路/Unfinished Path 包。两者是不同包，不应合并。共享词“未尽之路”在涅槃组中只是宣传引用（该 uploader 同时做两个包）。',
  },
  '一个小寂哦||__NOOP': { verdict: null, confidence: null, note: 'placeholder-never-used' },

  // ---- cluster 1: 一个小寂哦 mega-cluster. The scanner union-ed 16 groups via
  // 泰坦/星辉死神/幸运方块. Candidate layer already adjudicated the three real
  // false merges inside it; the cluster as a whole is NOT one pack.
  ...
  '墨竹ギ||墨竹ギ::quot 在失落的异界中 一个人生活下去 quot 生活 thelostworld 失落的异界|墨竹ギ::农业 低配 将 地下城 注入到 当中 史诗的地下城|墨竹ギ::史诗的地下城 dungeons of fantasy|墨竹ギ::史诗的地下城 一个 风 低配 极致优化 将 地下城 内容注入的高版本 啦|墨竹ギ::幻想的地下城 一个幻想风 风 低配 将 地下城 内容注入 的 啦|墨竹ギ::深渊之诗': {
    verdict: 'NOT_UNDER_MERGE', confidence: 'high',
  ...
  '小兜兜呀_||小兜兜呀_::原神与机械 的时代|小兜兜呀_::小兜兜 恭喜我的原神与机械 的时代 在bbs 资源站 量破6666次和在红石中继站浏览量达到400人 次 以及 相关 计划': {
    verdict: 'UNDER_MERGE', confidence: 'high',
    note: '确认：第二条是同UP主给自己的《原神与机械的时代》数据的“里程碑公告”视频，与主包是同一逻辑包却被拆成独立组 → 典型跨组拆分。',
  },
  'Terminus_54-32||terminus_54-32::怪物大乱斗 简介|terminus_54-32::新泰坦生物 new titan creatures mod pack released|terminus_54-32::泰坦 新增神器氪金萝莉|terminus_54-32::泰坦 看简介|terminus_54-32::泰坦 附 链接|terminus_54-32::泰坦生物 小': {
    verdict: 'UNDER_MERGE', confidence: 'medium',
    note: '确认（低风险）：6 组标题均为该 uploader 的“怪物大乱斗 / 泰坦生物 / 泰坦”系列，且多以“看简介/附链接/简介”结尾——是同一包反复发布被拆散。',
  },
  ...
  'Pork猪排||pork猪排::化龍 金鳞岂是池中物 一遇风云便化龙|pork猪排::化龍 麒麟踏雪|pork猪排::化龙 版本 龙可是帝王之征|pork猪排::龙语 连招 化龍 版本': {
    verdict: 'UNDER_MERGE', confidence: 'medium',
    note: '确认：4 组均为《化龍》同一包的宣传/更新（含繁体 化龍 与简体 化龙 两种写法，归一化后同根）→ 被拆散，属跨组拆分。',
  },
  ...
  '墨言eclipse||墨言eclipse::命轮无章 infinite random|墨言eclipse::颠覆性的|墨言eclipse::高版本沉浸 lt 未尽之诗 gt 官方宣传片 一首写给高版本 的远征诗': {
    verdict: 'NOT_UNDER_MERGE', confidence: 'high',
    note: '否决：命轮无章 / 摄影奇境与千界万锻（由“颠覆性的”误合）/ 未尽之诗 是不同包。此簇正印证了“颠覆性的”是真人误合并，而不是真实跨组同包。',
  },
  ...
  '啊liu22||啊liu22::更真实的 亲历人类 进化史 群峦传说 野望|啊liu22::简单群峦': {
    verdict: 'NOT_UNDER_MERGE', confidence: 'medium',
    note: '否决：「群峦野望」与「简单群峦」是两个不同包（都是群峦主题，但命名与定位不同）。簇仅由“群峦传说”主题词连通。',
  },
  ...
  '-阳春面面-||-阳春面面-::大 蔚蓝档案 版本|-阳春面面-::青春复兴': {
    verdict: 'UNDER_MERGE', confidence: 'medium',
    note: '确认（需注意）：候选层已把「青春复兴」判为 AMBIGUOUS（格雷青春版 vs 蔚蓝档案超大型）。本簇把“蔚蓝档案版本”与“青春复兴”连起来，倾向于认为这是同一包被拆散，但因 AMBIGUOUS 性质，保守标 medium。',
  },
...
/** The 5 group keys the 涅槃 pack was split into (hand-verified in 3G-F-A).
 *  The task requires ALL FIVE be surfaced as one logical under-merge, so the
 *  report asserts coverage against this list rather than against the clusters.
 *
 *  PER-KEY under-merge resolution. A cluster is a connected component, so one
 *  cluster can mix a genuine under-merge with an unrelated group (see the
 *  未尽之路 bridge). Coverage is therefore asserted at KEY granularity, and a
 *  key only counts when a finding covering it was adjudicated UNDER_MERGE -
 *  either via its whole cluster or via this explicit per-key list. */
...
// Hand-adjudicated: this key belongs to the 涅槃 pack.
...
function main() {
  ...
  const known7 = [
    '一个小寂哦::星辉死神', '一个小寂哦::四叶草', '一个小寂哦::各大主播同款',
    '墨言eclipse::颠覆性的', '原界环::or not',
    '叙利亚自爆民兵::voxy', 'tibsalta::难度驱动',
  ];
  const known7Missing = known7.filter((k) => !rows.some((r) => r.group_key === k && r.verdict === 'REAL_FALSE_MERGE'));
  // The 涅槃 under-merge must be visible as ONE logical pack split five ways.
  // Two detector clusters cover the five keys (4 + 1); coverage is asserted
  // against the hand-verified key list, not against either cluster alone.
  ...
  const nieKeysMissing = NIE_SPLIT_GROUP_KEYS.filter((k) => !coveredByUnderMerge.has(k));
  ...
  const result = {
    ...
    counts: {
      ...
      known_7_detected: known7.length - known7Missing.length,
      known_7_missing: known7Missing,
    },
    ...
    under_merge: {
      ...
      nie_expected_group_keys: NIE_SPLIT_GROUP_KEYS.length,
      nie_group_keys_covered: nieKeysCovered,
  ...
  console.log(`known 7 detected      : ${known7.length - known7Missing.length}/${known7.length}`);
  if (known7Missing.length) console.log(`!! MISSING KNOWN 7: ${known7Missing.join(', ')}`);
  ...
  console.log(`涅槃 group keys covered: ${nieKeysCovered.length}/${NIE_SPLIT_GROUP_KEYS.length}`);
  ...
  return (unadjudicated.length || known7Missing.length || nieKeysMissing.length) ? 1 : 0;
}
