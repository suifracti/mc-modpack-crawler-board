"""
BBSMC Legacy Exporter for Architecture V2.
Exports bbsmc_data.js strictly from build/canonical.db.
"""
import json
import sqlite3
...
class BBSMCExporter(BaseLegacyExporter):
    platform_name = "bbsmc"
    ...
    def export_all(self) -> Dict[str, Any]:
        ...
        try:
            ...
            loaders_cur = conn.execute("""
                ...
                WHERE sl.source_item_id LIKE 'bbsmc:%'
            """)
            ...
            mc_cur = conn.execute("""
                ...
                WHERE r.source_item_id LIKE 'bbsmc:%'
            ...
            # 5. Fetch latest release version name
            rel_cur = conn.execute("""
                SELECT source_item_id, version_name
                FROM releases
                WHERE source_item_id LIKE 'bbsmc:%' AND is_latest = 1
            """)
            latest_rel_map: Dict[str, str] = {}
            for row in rel_cur.fetchall():
                latest_rel_map[row["source_item_id"]] = row["version_name"]

            # 6. Fetch download links
            dl_cur = conn.execute("""
                SELECT source_item_id, link_type, url, label, extract_code
                FROM download_links
                WHERE source_item_id LIKE 'bbsmc:%'
            """)
            ...
            for it in items:
                ...
                rec = {
                    ...
                    "latest_version": latest_rel_map.get(si_id, ""),
                    "featured_gallery": extra.get("featured_gallery") or [],
                    "gallery": extra.get("gallery") or [],
                    "versions_data": extra.get("versions_data") or [],
                    "download_links": dl_map.get(si_id, []),
                    "has_server": has_server,
                }
                records.append(rec)

            out_path = self.write_sidecar("bbsmc_data.js", "bbsmcModpacksData", records)
            return {
                "platform": "bbsmc",
                "count": len(records),
                "file": out_path,
