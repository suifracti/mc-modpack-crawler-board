"""
Modrinth Legacy Exporter for Architecture V2.
Exports modrinth_data.js strictly from build/canonical.db.
"""
import json
import sqlite3
...
class ModrinthExporter(BaseLegacyExporter):
    ...
    def export_all(self) -> Dict[str, Any]:
        ...
        try:
            ...
            loaders_cur = conn.execute("""
                ...
                WHERE sl.source_item_id LIKE 'modrinth:%'
            """)
            ...
            mc_cur = conn.execute("""
                ...
                WHERE r.source_item_id LIKE 'modrinth:%'
            ...
            dl_cur = conn.execute("""
                ...
                FROM download_links
                WHERE source_item_id LIKE 'modrinth:%'
            """)
            ...
            claim_cur = conn.execute("""
                ...
                WHERE source_item_id LIKE 'modrinth:%' AND side = 'server'
            """)
            ...
            for it in items:
                ...
                has_server = (extra.get("server_side") in ("required", "optional")) or self.derive_legacy_has_server(c_status, c_cert)
                ...
                rec = {
                    ...
                    "client_side": extra.get("client_side") or "unknown",
                    "server_side": extra.get("server_side") or "unknown",
                    "env_display": extra.get("env_display") or "",
            ...
            out_path = self.write_sidecar("modrinth_data.js", "modrinthModpacksData", records)
            return {
                "platform": "modrinth",
                "count": len(records),
                "file": out_path,
